...
test("quando a atualização falha, a tela nomeia a versão certa, mostra o log e dá saída", async ({
  ...
  await expect(
    page.getByText("bash setup-kit/update.sh --to v1.0.0 --force"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("bash setup-kit/update.sh --to v1.1.0 --force"),
  ).toBeVisible();
...
test("quando o host não conseguiu comparar, a tela não diz que está em dia", async ({
  ...
  await expect(page.getByText("bash setup-kit/update.sh")).toBeVisible();
