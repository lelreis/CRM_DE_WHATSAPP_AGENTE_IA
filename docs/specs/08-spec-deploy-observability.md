...
|---|---|---|---|---|---|
| Production | `main` | `app.deskcomm.com.br` + `admin.deskcomm.com.br` | projeto Pro `deskcomm-prod` | `waha.deskcomm.com.br` (Hostinger) | `production` |
| Staging | `staging` | `staging.deskcomm.com.br` | projeto Free `deskcomm-staging` | `waha-staging.deskcomm.com.br` (Hostinger mesmo VPS, container separado) | `staging` |
...
#### Hostinger VPS (WAHA Plus + Nginx + Let's Encrypt)
...
└────────────┬───────────────────────────────────────────┬─────────────────┘
   ...
   └─────────┬────────────┘                    └────────────┬────────────┘
             ...
             ▼                                              ▼
   ┌──────────────────────┐                    ┌─────────────────────────┐
   │  Vercel Functions    │                    │  Hostinger VPS (São Paulo)│
   │  - Next.js App       │                    │  ┌──────────────────┐   │
...
| Hostinger VPS down (WAHA) | 7d (snapshot semanal) | 2h | Subir docker-compose em VPS reserva (restic restore + cloud-init); re-QR todas sessões NÃO afetadas se sessions restoradas |
...
| VPS Hostinger Turing | mensal | ~R$140 (~$28) | WAHA Plus + Nginx; datacenter São Paulo (latência baixa pro WhatsApp BR) |
