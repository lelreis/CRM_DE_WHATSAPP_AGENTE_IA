<!-- traduzido-de: docs/white-label.md@bea2659e9c40 -->
...
> ⚠️ **Changing the color from the screen afterwards does NOT rewrite the access e-mails.** Their text lives inside Supabase (GoTrue), not in the CRM, and what pushes it there is `marca-emails.sh` — which reads the **`.env`**, not the database. For the e-mails to follow a color changed in `/admin/marca`: adjust `APP_ACCENT_HEX` in the `.env` as well and run `bash setup-kit/marca-emails.sh`. This is why the installer interview matters: it is the only moment when both ends are born identical without anyone having to know about this.
...
Every installation ships the scripts in `setup-kit/`:
...
Full installation guide: [`setup-kit/README.md`](../setup-kit/README.md).
