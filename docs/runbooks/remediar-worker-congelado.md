...
```bash
cd /caminho/do/projeto && bash setup-kit/diagnostico.sh
```
...
2. **A partir da PRÓXIMA release, o agente completa parte disso sozinho — em até 5 minutos.**
   ...
   > `main`. Confira antes de prometer: `git show <tag>:setup-kit/_common.sh`.
...
```bash
cd /caminho/do/projeto
bash setup-kit/diagnostico.sh | tee /root/antes-remediacao.txt
...
O banco **não** volta com isso. Para voltá-lo, `bash setup-kit/restore.sh` com o
dump que o A3 gerou — e ele pede confirmação digitada, de propósito.

### A4. Verificar que o worker mudou de verdade `[ENSAIADO]`

```bash
bash setup-kit/diagnostico.sh          # esperado: exit 0, "NÃO está afetada"
