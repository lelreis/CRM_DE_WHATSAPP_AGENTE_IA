...
(`IMG_NS` em `setup-kit/_common.sh`), mais o default de cada serviço em
`docker-compose.prod.yml`, mais `.env.vps.example`, os testes que casam a string
(`tests/shell/update-guard.test.sh`, `setup-kit/test-validators.sh`,
