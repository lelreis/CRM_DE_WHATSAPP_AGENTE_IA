...
11. **Hospedagem WAHA** (Railway no MVP, VPS Hostinger em produção; BYO documentado como Fase 2)
...
- SLA WAHA upstream: 99% (Railway/Hostinger não dão SLA forte; aceito como tradeoff)
...
### Externas
- **WAHA Plus** — instância hospedada (Railway $5-10/mês no MVP; VPS Hostinger plano Turing ~R$140/mês em produção com Nginx + Let's Encrypt; datacenter São Paulo)
...
12. **Estratégia de migração entre Railway (MVP) e Hostinger VPS (produção)** sem downtime de sessão
